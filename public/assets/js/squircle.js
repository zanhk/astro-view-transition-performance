...
class SquircleClass {
	...
	paint(ctx, geom, properties) {
		...
		const squircleSmooth = Number.parseFloat(
		...
		let squircleRadii = individualRadiiProps.map((prop) => {
			...
			return value ? Number.parseInt(value, 10) * distanceRatio : Number.NaN;
		});
		let shorthand_R;
		if (squircleRadii.some(Number.isNaN)) {
			...
			if (matches) {
				shorthand_R = matches.map(
					(val) => Number.parseInt(val, 10) * distanceRatio,
				);
		...
		const squrcleOutline = Number.parseFloat(
